...
async function handleTextStatus(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  ...
  if (code !== twilioConf.apiCode) {
    ...
    if ([
      'undelivered',
      'delivered',
    ].indexOf(eventData.MessageStatus) !== -1) {
...
function dateToString(date: Date): string {
  return `${date.getUTCFullYear()}-${(date.getUTCMonth() + 1).toString().padStart(2, '0')}-${date.getUTCDate().toString()
    .padStart(2, '0')}`;
}
