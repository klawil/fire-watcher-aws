...
const anyAdminTextTypes: MessageType[] = [
  'page',
  'transcript',
  'pageAnnounce',
];
...
async function getStats(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  ...
  if (
    ...
    [
      'y',
      'n',
    ].indexOf(event.queryStringParameters.live) === -1
  ) {
    ...
    (response as any) = { // eslint-disable-line
      ...
      data: await cloudWatch.getMetricData(metricRequest).promise()
        .then(metricsResponse => {
          if (typeof metricsResponse.MetricDataResults === 'undefined') return {
            names: {},
            data: [],
          };
          ...
          metrics.data = metricsResponse.MetricDataResults
            .reduce((
              agg: {
                ts: string; values: { [key: string]: number; }
              }[],
              item
            ) => {
